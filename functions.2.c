...
// ------------------------------------------------------------ //
    void InitialCond(MeshGrid * mesh, IntegerPoints * IP)
    {
    	...
    	printf("M_INF= %lf; U_INF= %lf; rho_inf=%lf\n",IP->M_INF,U_INF,rho_inf);
        for(i=0;i<IMAX;i++)
        {
...
// ------------------------------------------------------------ //
    void ReadMesh(MeshGrid * mesh, char * filename, IntegerPoints * IP, HalfPoints * HP, ThomStr * T)
    {
    	...
    	for ( i = IMAX-1 ; i >= 0; i--)
    	{
    		for ( j = JMAX-1; j >= 0; j--) 
    		{
    			fscanf( fr, "%lf %lf", &mesh->x[i][j], &mesh->y[i][j] );
    		}
    	}
...
// ------------------------------------------------------------ //
    ...
    {	
    	...
    	// interior point at the external boundary
    	else if(i>0 && i<IMAX-1 && j==0) 
    	{
	       ...
	       // IP->x_eta[i][j] = 0.25*(-3.0*mesh->x[i][j] + 4.0*mesh->x[i][j+1] - mesh->x[i][j+2]);  // x_eta(i,j)
	       // IP->y_eta[i][j] = 0.25*(-3.0*mesh->y[i][j] + 4.0*mesh->y[i][j+1] - mesh->y[i][j+2]);  // y_eta(i,j)
	        IP->x_eta[i][j] = (mesh->x[i][j+1] - mesh->x[i][j]);  // x_eta(i,j)
	        IP->y_eta[i][j] = (mesh->y[i][j+1] - mesh->y[i][j]);  // y_eta(i,j)
	       ...
	       // HP->yhi_eta[i][j] = 0.5*(-3.0*(mesh->y[i+1][j]+mesh->y[i][j]) +4.0*(mesh->y[i+1][j+1]+mesh->y[i][j+1]) -(mesh->y[i+1][j+2] + mesh->y[i][j+2]));  // y_eta(i+1/2,j)	
	        ...
	        HP->xhi_eta[i][j] = 0.5*(mesh->x[i+1][j+1] + mesh->x[i][j+1] - mesh->x[i+1][j] - mesh->x[i][j]);  // x_eta(i+1/2,j)
	        HP->yhi_eta[i][j] = 0.5*(mesh->y[i+1][j+1] + mesh->y[i][j+1] - mesh->y[i+1][j] - mesh->y[i][j]);  // y_eta(i+1/2,j)
    	}
    	else if(i==0 && j==0)
    	{
    		IP->x_csi[i][j] = 0.5*(mesh->x[i+1][j] - mesh->x[IMAX-2][j]);  // x_csi(i,j) 
    	...
    	else if(i==IMAX-1 && j <JMAX-1)
    	{
		    ...
		    //  IP->y_eta[i][j] = 0.25*(-3.0*mesh->y[i][j] + 4.0*mesh->y[i][j+1] - mesh->y[i][j+2]);  // y_eta(i,j) 
                ...
                HP->xhi_eta[i][j] = 0.5*(mesh->x[1][j+1] + mesh->x[i][j+1] - mesh->x[1][j] - mesh->x[i][j]);  // x_eta(i+1/2,j)
	            HP->yhi_eta[i][j] = 0.5*(mesh->y[1][j+1] + mesh->y[i][j+1] - mesh->y[1][j] - mesh->y[i][j]);  // y_eta(i+1/2,j)
    		}
    		else
    		{
    			IP->x_eta[i][j] = 0.5*(mesh->x[i][j+1] - mesh->x[i][j-1]);  // x_eta(i,j)
    	...
    	else if(i<IMAX-1 && j==JMAX-1)
    	{
	       ...
	       // IP->x_eta[i][j] = 0.25*(3.0*mesh->x[i][j] - 4.0*mesh->x[i][j-1] + mesh->x[i][j-2]);  // x_eta(i,j)
	       // IP->y_eta[i][j] = 0.25*(3.0*mesh->y[i][j] - 4.0*mesh->y[i][j-1] + mesh->y[i][j-2]);  // y_eta(i,j) 
	        IP->x_eta[i][j] = (mesh->x[i][j] - mesh->x[i][j-1]);  // x_eta(i,j)
	        IP->y_eta[i][j] = (mesh->y[i][j] - mesh->y[i][j-1]);  // y_eta(i,j)
	       ...
	       // HP->xhi_eta[i][j] = 0.5*(3.0*(mesh->x[i+1][j]+mesh->x[i][j]) -4.0*(mesh->x[i+1][j-1]+mesh->x[i][j-1]) +(mesh->x[i+1][j-2] + mesh->x[i][j-2]));  // x_eta(i+1/2,j)
	       // HP->yhi_eta[i][j] = 0.5*(3.0*(mesh->y[i+1][j]+mesh->y[i][j]) -4.0*(mesh->y[i+1][j-1]+mesh->y[i][j-1]) +(mesh->y[i+1][j-2] + mesh->y[i][j-2]));  // y_eta(i+1/2,j)		
	        HP->xhi_eta[i][j] = 0.5*(mesh->x[i+1][j] + mesh->x[i][j] - mesh->x[i+1][j-1] - mesh->x[i][j-1]);  // x_eta(i+1/2,j)
	        HP->yhi_eta[i][j] = 0.5*(mesh->y[i+1][j] + mesh->y[i][j] - mesh->y[i+1][j-1] - mesh->y[i][j-1]);  // y_eta(i+1/2,j)		
    	}
    	else if(i==IMAX-1 && j==JMAX-1)
    	{
    		IP->x_csi[i][j] = 0.5*(mesh->x[1][j] - mesh->x[i-1][j]);  // x_csi(i,j) 
	       ...
	       // HP->yhi_eta[i][j] = 0.5*(3.0*(mesh->y[1][j]+mesh->y[i][j]) -4.0*(mesh->y[1][j-1]+mesh->y[i][j-1]) +(mesh->y[1][j-2] + mesh->y[i][j-2]));  // y_eta(i+1/2,j)	
	        HP->xhi_eta[i][j] = 0.5*(mesh->x[1][j] + mesh->x[i][j] - mesh->x[1][j-1] - mesh->x[i][j-1]);  // x_eta(i+1/2,j)
	        HP->yhi_eta[i][j] = 0.5*(mesh->y[1][j] + mesh->y[i][j] - mesh->y[1][j-1] - mesh->y[i][j-1]);  // y_eta(i+1/2,j)
    	}
    	else
    	{
    		IP->x_csi[i][j] = 0.5*(mesh->x[i+1][j] - mesh->x[i-1][j]);  // x_csi(i,j) 
    ...
    void ConstSetCal(MeshGrid * mesh, HalfPoints * HP, IntegerPoints * IP)
    {
        ...
        fprintf(fw,"VARIABLES = \"X\", \"Y\", \"xhi_csi\", \"xhi_eta\", \"xhj_csi\", \"xhj_eta\", \"JacobianHi\", \"JacobianHj\"\n\
...
// ------------------------------------------------------------ //
    ...
    void HPfiDeriv(HalfPoints * HP, IntegerPoints * IP, int i, int j)
    {
    	...
    	else if(i>0 && j==0  && i<IMAX-1)
    	{
	       ...
	       // IP->fi_eta[i][j] = 0.25*(-3.0*IP->fi[i][j] + 4.0*IP->fi[i][j+1] - IP->fi[i][j+2]);
	        IP->fi_eta[i][j] = (IP->fi[i][j+1] - IP->fi[i][j]);

	        
	        HP->fi_csi_hi[i][j] = (IP->fi[i+1][j] - IP->fi[i][j]);
	       // HP->fi_eta_hi[i][j] = 0.25*(-3.0*(IP->fi[i+1][j]+IP->fi[i][j]) +4.0*(IP->fi[i+1][j+1]+IP->fi[i][j+1]) -(IP->fi[i+1][j+2] + IP->fi[i][j+2]));
	        HP->fi_eta_hi[i][j] = 0.5*(IP->fi[i+1][j+1] + IP->fi[i][j+1] - IP->fi[i+1][j] - IP->fi[i][j]);
    	...
    	else if(i==IMAX-1 && j <JMAX-1)
    	{
	       ...
	       // 	HP->fi_eta_hi[i][j] = 0.25*(-(IP->fi[1][j]+IP->fi[i][j]) + (IP->fi[1][j+1]+IP->fi[i][j+1]));
	        }
	        else
	        {
	        	IP->fi_eta[i][j] = 0.5*(IP->fi[i][j+1] - IP->fi[i][j-1]);
	        ...
	        HP->fi_eta_hi[i][j] = 0.5*(IP->fi[1][j+1] + IP->fi[i][j+1] - IP->fi[1][j] - IP->fi[i][j]);
    	...
    	else if(i<IMAX-1 && j==JMAX-1)
    	{
	       ...
	       // IP->fi_eta[i][j] = 0.25*(3.0*IP->fi[i][j] -4.0*IP->fi[i][j-1] + IP->fi[i][j-2]);
	       // IP->fi_eta[i][j] = (IP->fi[i][j] - IP->fi[i][j-1]);
    	...
    	else
    	{
	        ...
	        // nao preciso calcular esses dois em JMAX-1, somente ate JMAX-2   
	        HP->fi_csi_hj[i][j] = 0.25*((IP->fi[i+1][j+1] - IP->fi[i-1][j+1]) + (IP->fi[i+1][j] - IP->fi[i-1][j])); 
	        HP->fi_eta_hj[i][j] = (IP->fi[i][j+1] - IP->fi[i][j]);
    	}
...
// ------------------------------------------------------------ //
    void Residue(HalfPoints * HP, IntegerPoints * IP, int n)
    {
    	...
    	if(n>0)
    	{
    		IP->resMax = (double *) realloc(IP->resMax, (n+1)*sizeof(double));
	    	IP->resL1  = (double *) realloc(IP->resL1 , (n+1)*sizeof(double));
	    	IP->resL2  = (double *) realloc(IP->resL2 , (n+1)*sizeof(double));
    	}
        ...
        for(i=0;i<=IMAX-1;i++)
        {
            for(j=1;j<=JMAX-1;j++)
            {
            	...
            	{
            		IP->residue[i][j] = ((HP->rho_til_hi[i][j]*HP->U_hi[i][j]/HP->J_hi[i][j]) - (HP->rho_til_hi[i-1][j]*HP->U_hi[i-1][j]/HP->J_hi[i-1][j])) + (-2.0*(HP->rho_bar_hj[i][j-1]*HP->V_hj[i][j-1]/HP->J_hj[i][j-1]));
            	}
            	else if(j==JMAX-1 && i==0)
            	{
            		IP->residue[i][j] = ((HP->rho_til_hi[i][j]*HP->U_hi[i][j]/HP->J_hi[i][j]) - (HP->rho_til_hi[IMAX-2][j]*HP->U_hi[IMAX-2][j]/HP->J_hi[IMAX-2][j])) + (-2.0*(HP->rho_bar_hj[i][j-1]*HP->V_hj[i][j-1]/HP->J_hj[i][j-1]));
            	}
            	else
            	{
...
// ------------------------------------------------------------ //
    ...
    void IterMethod(IntegerPoints * IP, HalfPoints * HP, ThomStr * T, int iter)
    {
    	...
    	if(iter>0) IP->beta = (double *) realloc(IP->beta, (iter+1)*sizeof(double));
        ...
        for(j=1;j<=JMAX-1;j++)
        {
            ...
            Thomas_Algorithm(IP->corr,T,j,1,IMAX,JMAX);
        }
...
// ------------------------------------------------------------ //
    void Output(MeshGrid * mesh, IntegerPoints * IP)
    {
        ...
        //fprintf(fw,"    x          y         rho         fi         corr          U         V\n");
        fprintf(fw,"VARIABLES = \"X\", \"Y\", \"rho\", \"fi\", \"corr\", \"U\", \"V\", \"Jacobian\" \n\
ZONE I=%d, J=%d, F=POINT\n", IMAX, JMAX);
        ...
        for(j=0;j<JMAX;j++)
        {
    	    for(i=0;i<IMAX;i++)
    	    {
                ...
                mesh->x[i][j],mesh->y[i][j],IP->rho[i][j],IP->fi[i][j],
                IP->corr[i][j],IP->U[i][j],IP->V[i][j], IP->J[i][j]);
            }
        }
        fclose(fw);
    }
