...
// ------------------------------------------------------------ //
    void AllocateAll(MeshGrid * mesh, HalfPoints * HP, IntegerPoints * IP)
    {
        ...
        HP->rhi_bar_hj = calloc(IMAX, sizeof(double *));
        ...
        for(i = 0; i < IMAX; i++) { 
            ...
            HP->rhi_bar_hj[i] = calloc(JMAX, sizeof(double));
...
// ------------------------------------------------------------ //
    ...
    void HPderivPM(MeshGrid * mesh, IntegerPoints * IP, HalfPoints * HP, int i, int j)
    {
    ...
    void HPderivNM(IntegerPoints * IP, HalfPoints * HP, int i, int j)
    {
    ...
    void ConstSetCal(MeshGrid * mesh, HalfPoints * HP, IntegerPoints * IP)
    {
        ...
        for(i=1;i<IMAX-1;i++)
        {
            for(j=1;j<JMAX-1;j++)
            {
                ...
                HPderivPM(mesh,IP,HP,i,j); // Integer-Point/Half-Point derivatives on Physical Mesh
                ...
                HPderivNM(IP,HP,i,j); // Integer-Point/Half-Point derivatives on Numerical Mesh
    ...
    void Residue(HalfPoints * HP, IntegerPoints * IP)
    {
        ...
        for(i=1;i<IMAX-1;i++)
        {
            for(j=1;j<JMAX-1;j++)
            {
                ...
                IP->resMax = IP->resMax + fabs(IP->residue[i][j]);
            }
        }
    ...
    void IterMethod(IntegerPoints * IP, HalfPoints * HP, ThomStr * T, int iter)
    {
        ...
        for(i=0;i<IMAX;i++)
        {
            for(j=JMAX-1;j>0;j--)
            {
                ...
                Aj1 = HP->rho_bar_hj[i][j-2]*HP->A3_hj[i][j-2]/HP->J_hj[i][j-2];
